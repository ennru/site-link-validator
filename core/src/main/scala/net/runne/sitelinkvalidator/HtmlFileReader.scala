...
object HtmlFileReader {
  ...
  def reader(reporter: ActorRef[Reporter.Messages],
             anchorValidator: ActorRef[AnchorValidator.Messages],
             urlTester: ActorRef[UrlTester.Messages],
             linkCollector: ActorRef[LinkCollector.Messages]): Behavior[Messages] = Behaviors.receive {
          ...
          val fileReader: Future[Done] = FileIO
            ...
            .mapConcat {
              case s: StartElement if s.localName == "a" =>
                s.attributes.get("href").map(Link).toIndexedSeq ++ s.attributes
                  .get("name")
                  .map(Anchor)
                  .toIndexedSeq ++ s.attributes.get("id").map(Anchor).toIndexedSeq
              ...
              case s: StartElement if s.localName == "script" && s.attributes.contains("src") =>
                s.attributes.get("src").map(Link).toIndexedSeq
              case _ =>
                immutable.Seq.empty
            }
